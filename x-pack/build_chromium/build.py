...
print(f'Building Chromium {source_version} for {arch_name} from {src_path}')
print(f'src path: {src_path}')
print('depot_tools path: ' + path.join(build_path, 'depot_tools'))
print(f'build_chromium_path: {build_chromium_path}')
print(f'args.gn file: {argsgn_file}')
print
...
checked_out = runcmdsilent(f'git checkout build-{base_version}')
if checked_out != 0:
  print('Syncing remote version')
  runcmd(f'git fetch origin {source_version}')
  print('Creating a new branch for tracking the source version')
  runcmd(f'git checkout -b build-{base_version} {source_version}')
...
print(f'Updating PATH for depot_tools: {full_path}')
...
if platform.system() == 'Linux':
  if arch_name:
    ...
    runcmd(
        f'{src_path}/build/linux/sysroot_scripts/install-sysroot.py --arch={arch_name}'
    )
  print('Running install-build-deps...')
  runcmd(f'{src_path}/build/install-build-deps.sh')
...
runcmd(f'cp {argsgn_file} {argsgn_file_out}')
...
base_filename = f'out/headless/chromium-{base_version}-{platform.system().lower()}_{arch_name}'
zip_filename = f'{base_filename}.zip'
md5_filename = f'{base_filename}.md5'

print(f'Creating {path.join(src_path, zip_filename)}')
...
def archive_file(name):
  ...
  to_path = path.join(
      f'headless_shell-{platform.system().lower()}_{arch_name}', name)
...
print(f'Creating {path.join(src_path, md5_filename)}')
with open (md5_filename, 'w') as f:
  f.write(md5_file(zip_filename))

runcmd(
    f'gsutil cp {path.join(src_path, zip_filename)} gs://headless_shell_staging'
)
runcmd(
    f'gsutil cp {path.join(src_path, md5_filename)} gs://headless_shell_staging'
)
